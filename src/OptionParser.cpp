...
Option& OptionContainer::add_option(const std::string& opt) {
...
Option& OptionContainer::add_option(const std::vector<std::string>& v) {
...
std::string OptionContainer::format_option_help(unsigned int indent /* = 2 */) const {
  stringstream ss;
...
OptionParser& OptionParser::add_option_group(const OptionGroup& group) {
  for (list<Option>::const_iterator oit = group._opts.begin(); oit != group._opts.end(); ++oit) {
    ...
    for (set<std::string>::const_iterator it = option._short_opts.begin(); it != option._short_opts.end(); ++it)
      _optmap_s[*it] = &option;
    for (set<std::string>::const_iterator it = option._long_opts.begin(); it != option._long_opts.end(); ++it)
      _optmap_l[*it] = &option;
  }
...
const Option& OptionParser::lookup_long_opt(const std::string& opt) const {
  ...
  if (matching.size() > 1) {
    std::string x = str_join_trans(", ", matching.begin(), matching.end(), str_wrap("--", ""));
...
void OptionParser::error(const std::string& msg) const {
  print_usage(cerr);
  cerr << prog() << ": " << _("error") << ": " << msg << std::endl;
  exit();
}
////////// } class OptionParser //////////
